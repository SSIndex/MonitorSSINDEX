    ...
    [
        html.Div(
            [
                ...
                html.Img(
                    src='assets/ssindex.png', style={'height': '60px', 'margin': '10px'}
                ),
                html.H1(
                    "Monitor SSINDEX",
                    id="title",
...
def update_graphs(selected_year, selected_pilar, selected_banks, n_clicks, style):
    ...
    bubble_fig = px.scatter(
        ...
        color='Bank Name',
        title='Bubble Chart of Sentiments',
    )
...
df_grouped = (
    df_filtered.groupby(['Bank Name', pd.Grouper(key='ds', freq='M')])[
        'Nuevo_Sentiment_Score'
    ]
    .mean()
    .reset_index()
...
heatmap_fig.update_layout(title='Heatmap of Average Sentiment Scores', xaxis_nticks=36)
...
@app.callback(Output('review-table', 'data'), Input('heatmap', 'clickData'))
def display_reviews(clickData):
    ...
    filtered_df = df1[
        (df1['ds'].dt.to_period('M') == pd.Period(date, freq='M'))
        & (df1['Bank Name'] == bank_name)
    ]
