...
fig_pie = px.pie(source_data, values='counts', names='data_source', title=f'Data Source Distribution for {bkn}')
category_order = ['Very Low', 'Low', 'Medium', 'High', 'Very High']

hist_data = aux.groupby(['New_Sentiment_Score']).size().reset_index(name='counts')
hist_data['New_Sentiment_Score'] = pd.Categorical(hist_data['New_Sentiment_Score'], categories=category_order, ordered=True)
                            ...
                            children=[
                                ...
                                dbc.Row(
                                    [
                                        ...
                                        dbc.Col(
                                            dcc.Graph(
                                                id="gauge-chart", figure=initial_gauge_chart
                                            ),
                                            width=6,
                                        ),
                                    ]
