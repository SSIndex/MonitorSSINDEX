...
app.layout = dbc.Container([
    ...
    dbc.Col([
        html.H5("Choose the bank(s) you want to compare:", style={'margin-top': '50px'}),
        dbc.Row([
            dbc.Col([
                dcc.Checklist(
                    id='bank-checklist',
    ...
    dbc.Col([
        dbc.Row([
            dbc.Col([
                html.Div("Total Bank Score", style={'font-size': '30px', 'font-weight': 'bold', 'margin': '0px'}),
            ...
            dbc.Col([
                html.Img(id='sentiment-image', style={'height': '150px','width':'150px', 'margin-left': '50px'})
            ], width=12),
    ...
    dbc.Row([
        dbc.Col(html.Div(id='review-table-container'), width=12)
    ]),
    dbc.Row([
        ...
        dbc.Row([
        dbc.Col(dcc.Graph(id='overall-time-line-plot'), width=12)
    ]),
    dbc.Row([
        dbc.Col(dcc.Graph(id='sentiment-heatmap'), width=12)
    ]),
    dbc.Row([
...
@app.callback(
    Output('sentiment-image', 'src'),
    Input('average-bank', 'children')
)
def update_image(average_bank):
    if average_bank is None:
        return 'assets/neutral.png'
    average_bank = float(average_bank)
    if average_bank > 50:
        return 'assets/happy.jpg'
    elif average_bank < 45:
        return 'assets/angry.jpg'
    else:
        return 'assets/neutral.png'
...
def update_line_chart(selected_year, selected_pilar, selected_bank):
    ...
    fig = px.line(filtered_df, x='date', y='Total_Sentiment_Score', title=f'Sentiment Over {selected_year} for {selected_bank} in {selected_pilar}')
    return fig

@app.callback(
    [Output('us-map', 'figure'), Output('review-table-container', 'children'),Output('average-bank', 'children')],
    [Input('year-dropdown', 'value'),
     Input('pilar-dropdown', 'value'),
     Input('bank-dropdown', 'value'),
...
def update_us_map(selected_year, selected_pilar, selected_bank, click_data):
    fil_df=df1[(df1['year'] == selected_year) & 
    ...
    return fig, reviews_table,ave_bank
